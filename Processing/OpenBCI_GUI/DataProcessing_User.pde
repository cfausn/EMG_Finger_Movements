...
class DataProcessing_User {
  ...
  public void process(float[][] data_newest_uV, //holds raw bio data that is new since the last call
    ...
    if(isReadyToTrain){
      if(millis() - trialStart >= 5000 && millis() - trialStart <= 5300){
        output("Move Index Finger");
        isTrial = true;
      }
      else if (millis() - trialStart >= 7000 && millis() - trialStart <= 7300){
        output("Done recording trial...");
        isTrial = false;
      }
      else if (millis() - trialStart >= 10000 && millis() - trialStart <= 10300){
        trialCount++;
        output("Prepare for trial number " + trialCount);
        trialStart = millis();
      }
          //trialStart = millis();
    }
